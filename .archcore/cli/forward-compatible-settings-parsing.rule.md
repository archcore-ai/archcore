...
- `@cli/internal/config/config.go` — `Settings.Extra`, `knownFields`, `UnmarshalJSON`, `MarshalJSON`,
  `UnknownFieldNames`.
- `@cli/cmd/config_warn.go` — `warnUnknownConfigFields`, called from `mcp`, `config`, `doctor`, `sync`.
