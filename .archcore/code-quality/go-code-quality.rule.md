...
- Build the report as data and render it in a separate function — the boundary-rendering rule states the obligation. `@cli/cmd/status_report.go` records why the resolution has to happen at call time.
...
- Call `signal.NotifyContext()` at the top level (`@cli/main.go`) for graceful shutdown on SIGINT and SIGTERM.
