...
`~/.claude/settings.json`. The suite stayed green throughout — @cli/internal/testsupport/isolate.go.
...
State which of the four this package actually reaches, and why. @cli/cmd/main_test.go names all four
...
matter what — @cli/cmd/isolation_guard_test.go.
...
- **A new host CLI escapes the trap.** `hostCLIs` in @cli/internal/testsupport/isolate.go repeats the CLI
