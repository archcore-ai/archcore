...
- The repository targets `go 1.25.11` (@cli/go.mod), so idioms introduced in Go 1.26 and Go 1.27 do not apply and would be a defect if written today.
...
- Copying the catalog into `.archcore/` would add roughly 70 entries to the `code-quality` tag set, which `review-go` loads in full into the review context (@cli/.claude/skills/review-go/SKILL.md, Step 2). The context cost is paid on every review.
- The catalog goes stale at each Go release, and a copy inside `.archcore/` would need a maintainer.

### Known Constraints

- The module targets `go 1.25.11` (@cli/go.mod), so Go 1.26 and Go 1.27 entries are out of scope until the module version moves.
