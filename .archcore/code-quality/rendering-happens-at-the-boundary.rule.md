...
> `internal/wiring` — @cli/internal/wiring/wiring.go: "Results carry raw errors and absolute paths:
> rendering is the caller's job at its own boundary." `AgentResult` repeats it: "Paths are absolute;
> errors are raw."

**Good** — clause 5. `hostWiringExecutor` at @cli/cmd/host_wiring.go adapts `wiring.Apply` for the MCP
...
@cli/cmd/status_report.go. `writeTo` is the only place `display.*Line` is called.
