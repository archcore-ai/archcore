...
> `ErrPathReadOnlyGlobal`, `ErrPathNotDocument`, `ErrPathEscapes` at @cli/internal/docs/guard.go, whose
...
**Good** — clause 9. `git.ErrGitAbsent` at @cli/internal/git/git.go is distinct from "not a repository",
...
- `sanitizeError` at @cli/internal/mcp/tools/common.go is the rendering half at the MCP boundary and holds
  clause 5 for paths that reach a tool result.
- The write-guard tests in @cli/cmd/hook_write_guard_test.go pin clause 8 for the one surface where a
