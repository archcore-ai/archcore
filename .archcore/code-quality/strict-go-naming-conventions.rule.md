...
- **MUST NOT**: Suffix a struct with `-er` when the struct is a data record rather than a behavior. `hookMatcher` in `@cli/internal/wiring/hooks_agents.go` was the shape this forbids — a `{Matcher, Hooks}` JSON record that matched nothing — and was renamed to `hookMatcherGroup` in September 2026.
...
- `exhaustive` — flags a non-exhaustive `switch` **that has no `default` clause**. The config sets
  ...
  one of them — `GlobalState.Fatal` at `@cli/internal/docs/inspect.go` — depends on it to make a new
