...
3. Add the `TypeXxx` constant, the `categoryMap` row, and the `ValidTypes` entry in @cli/templates/templates.go.
4. Write `generateXxxTemplate` with visible placeholders and wire it into `GenerateTemplate`.
5. Add `RequiredSections`, `ProseProfiles`, and any `StepSections` or `ForeignSections` rows in @cli/templates/precision.go.
6. Add a body cap row in `MaxBodyLines` when the type carries one.
7. Add a per-type finding function in @cli/internal/advisory/precision.go only for checks the shared tables cannot express.
8. Decide the injection rank in @cli/internal/advisory/code_alignment.go; leave vision types out.
9. Extend the type lines in @cli/internal/mcp/tools/create_document.go and @cli/internal/mcp/tools/list_documents.go.
10. Extend the type list, WHEN TO CREATE, TYPE SELECTION RULES, relations, and status text in @cli/internal/mcp/server.go.
11. Add the type to `corpusTypes` in @cli/internal/testsupport/corpus.go on the correct side of the allowlist.
12. Add template, precision, alignment, search-ranking, cascade, and restatement tests beside each change.
13. Add a lifecycle test in @cli/internal/mcp/integration/ on the pattern of @cli/internal/mcp/integration/research_spec_test.go.
14. Update the counts and tables in @cli/README.md, the categories doc, the building doc, the free-form rule, and the advisory doc.
...
The actor-subject run: @cli/templates/templates.go (`TypeScenario`, `TypeJourney`), @cli/templates/precision.go (`SectionActors`, `ObservationSections`, `ActorStepSections`, `MaxBodyLines`), @cli/internal/advisory/precision.go (`actorSubjectFindings`), @cli/internal/advisory/precision_actor_subject_test.go, @cli/internal/mcp/integration/actor_subject_spec_test.go.
