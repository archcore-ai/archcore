...
The table is data, not prose: `ForeignSections` in `@cli/templates/precision.go` carries the heading-to-owner assignment, and the engines that read it are `@cli/internal/advisory/precision.go` and `@cli/internal/advisory/restatement.go`.
