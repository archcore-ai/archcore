...
Completed tasks below have source or verification evidence in @cli/docs/releases/research-validation.md. Release tasks remain open.

### Phase 1 — document types

This phase establishes vocabulary before relation scenarios depend on the new types.

1. [x] Register the two planned types in @cli/templates/templates.go.
2. [x] Add the research template generator in @cli/templates/templates.go.
3. [x] Add the evidence template generator in @cli/templates/templates.go.
4. [x] Wire both generators into the dispatcher in @cli/templates/templates.go.
5. [x] Register required sections and prose profiles in @cli/templates/precision.go.
6. [x] Update the creation vocabulary in @cli/internal/mcp/tools/create_document.go.
7. [x] Update discovery descriptions in @cli/internal/mcp/tools/list_documents.go.
8. [x] Document type selection, status conventions, and source tags in @cli/internal/mcp/server.go.
9. [x] Extend type-registration and generation coverage in @cli/templates/templates_test.go.
10. [x] Cover missing-section advisories in @cli/internal/advisory/precision_findings_test.go.
11. [x] Pin default ranking behavior in @cli/internal/mcp/tools/search_documents_test.go.
12. [x] Pin CodeAlignment exclusions in @cli/internal/advisory/code_alignment_test.go.

The ranking table in @cli/internal/mcp/tools/search_documents.go and selector in @cli/internal/advisory/code_alignment.go already supply the planned defaults. New tests guard those boundaries.

### Phase 2 — relation vocabulary

This phase extends stored relation values while retaining existing validation boundaries.

13. [x] Extend the relation registry in @cli/internal/sync/manifest.go.
14. [x] Update relation axes and direction descriptions in @cli/internal/mcp/tools/add_relation.go.
15. [x] Update removal vocabulary descriptions in @cli/internal/mcp/tools/remove_relation.go.
16. [x] Extend relation guidance in @cli/internal/mcp/server.go.
17. [x] Expand valid-value and refusal fixtures in @cli/internal/sync/manifest_test.go.
18. [x] Cover relation creation and refusal boundaries in @cli/internal/mcp/tools/add_relation_test.go.
19. [x] Cover deletion of new relation values in @cli/internal/mcp/tools/remove_relation_test.go.
20. [x] Verify relation readback in @cli/internal/mcp/tools/list_relations_test.go and @cli/internal/mcp/tools/get_document_test.go.
21. [x] Pin cascade exclusions in @cli/cmd/hook_post_tool_use_test.go.
22. [x] Pin restatement exclusions in @cli/internal/advisory/restatement_test.go.
23. [x] Add an isolated seven-value manifest fixture in @cli/cmd/doctor_test.go.

The existing exclusion sets remain in @cli/cmd/hook_post_tool_use.go and @cli/internal/advisory/restatement.go. New relations require no content propagation.

### Phase 3 — frontmatter preservation

This phase is independent of phases 1 and 2.

24. [x] Preserve ordered unknown YAML entries during parsing in @cli/templates/templates.go.
25. [x] Carry retained metadata through @cli/internal/mcp/tools/update_document.go.
26. [x] Serialize retained metadata through @cli/internal/mcp/tools/common.go.
27. [x] Adapt the shared serializer caller in @cli/internal/mcp/tools/create_document.go.
28. [x] Replace the ignored-unknown-fields fixture in @cli/templates/templates_test.go.
29. [x] Cover retained YAML serialization in @cli/internal/mcp/tools/common_test.go and refusal through @cli/internal/mcp/tools/update_document_test.go.
30. [x] Exercise the update matrix in @cli/internal/mcp/tools/update_document_test.go.
31. [x] Verify the structured JSON payload boundary in @cli/internal/sync/payload_test.go.
...
32. [x] Extend the lifecycle scenarios in @cli/internal/mcp/integration/ using the existing in-process harness.
33. [x] Exercise relation round trips in @cli/internal/mcp/integration/relations_test.go.
34. [x] Exercise metadata retention in @cli/internal/mcp/integration/forward_compat_test.go.
35. [x] Verify original `rnd` behavior through @cli/internal/mcp/integration/rnd_test.go.
...
38. [x] Update current type counts in @cli/README.md and @.archcore/cli-ui/building-the-cli.doc.md.
39. [x] Correct the stale count in @.archcore/cli/mcp-token-optimization.idea.md.
40. [x] Run the targeted package suites for @cli/templates/, @cli/internal/mcp/, @cli/internal/sync/, @cli/internal/advisory/, and @cli/cmd/.
41. [x] Run `go test ./...` from the repository root.
42. [x] Run `golangci-lint run ./...` against @cli/.golangci.yml.
43. [x] Build the CLI from @cli/main.go.

The in-process harness is @cli/internal/mcp/integration/harness_test.go. Tests use temporary repositories and isolated host state. A native-binary smoke test also verified category filters over stdio and seven-value manifest acceptance.

### Phase 5 — release handoff

This phase prepares the single release after verification.

44. [x] Prepare release notes through the workflow defined in @cli/.github/workflows/release.yml.
45. [x] Select the release tag consumed by @cli/.github/workflows/release.yml and @cli/.goreleaser.yaml.
46. [ ] Publish the verified release through @cli/.github/workflows/release.yml after release authorization.
47. [ ] Record the shipped engine version for the plugin compatibility handoff.

Release notes are prepared in @cli/docs/releases/research-release-candidate.md. The user selected `v0.8.3`; no tag or release was created. Version injection remains tag-driven through @cli/main.go and @cli/.goreleaser.yaml. The plugin minimum-engine gate belongs to the plugin repository.
...
The accepted document-model decision assigns templates to @cli/templates/ and document operations to @cli/internal/docs/. The accepted relation-store decision keeps persistence in @cli/internal/sync/manifest.go. Existing graph edges carry these dependencies.
...
Older binaries reject manifests containing new relation values — @cli/internal/sync/manifest.go. This release adds no data migration, downgrade conversion, or remote-server compatibility guarantee. Plugin routing follows the engine release. The shared category vocabulary was updated before publication at the user's request.
