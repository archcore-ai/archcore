...
The canon data lives in @cli/templates/precision.go: `RequiredSections`, `ProseProfiles`, `StepSections`, `ObservationSections`, `ActorStepSections`, `ForeignSections`, and the per-type body-cap table `MaxBodyLines`. The checker is @cli/internal/advisory/precision.go. The injector ranking is `alignmentTypePriority` in @cli/internal/advisory/code_alignment.go. The search ranking is `typePriority` in @cli/internal/mcp/tools/search_documents.go. The cascade set is `cascadeRelations` in @cli/cmd/hook_post_tool_use.go; the restatement set is `contentRelations` in @cli/internal/advisory/restatement.go.
...
Conformance requires every numbered obligation above. Regression coverage: @cli/internal/advisory/precision_actor_subject_test.go pins clauses 1 to 14, the fence and outer-pipe constraints, and the failure rules; @cli/internal/advisory/code_alignment_test.go pins clauses 15 and 16; @cli/internal/mcp/tools/search_documents_test.go pins clause 17; @cli/cmd/hook_post_tool_use_test.go and @cli/internal/advisory/restatement_test.go pin constraints 2 and 3.
