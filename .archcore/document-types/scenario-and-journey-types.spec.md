...
The registry and template dispatcher live in @templates/templates.go; the source-extension list lives in @templates/source_extensions.go. MCP creation and discovery use @internal/mcp/tools/create_document.go, @internal/mcp/tools/list_documents.go, and @internal/mcp/tools/search_documents.go; the server instructions live in @internal/mcp/server.go.
...
3. IF a binary that predates this release scans a `.scenario.md` or `.journey.md` file, THEN that binary skips the file in the scan and reports an invalid type in `status`; this is the current behavior of @internal/docs/scan.go and @cmd/status.go, and no migration changes it.

## Conformance

Conformance requires every numbered obligation above. Regression coverage includes @templates/templates_test.go, @templates/source_extensions_test.go, @internal/mcp/tools/create_document_test.go, and @internal/mcp/tools/list_documents_test.go. Lifecycle scenarios in @internal/mcp/integration/ cover creation, listing, retrieval, and search for both types.
