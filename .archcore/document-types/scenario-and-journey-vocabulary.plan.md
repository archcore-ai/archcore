...
1. [x] Register `TypeScenario` and `TypeJourney` with their categories in @templates/templates.go.
2. [x] Add both names to `ValidTypes` in @templates/templates.go.
3. [x] Add `generateJourneyTemplate` and `generateScenarioTemplate` in @templates/templates.go.
4. [x] Wire both generators into `GenerateTemplate` in @templates/templates.go.
5. [x] Add `.feature` to the source-extension list in @templates/source_extensions.go.
6. [x] Extend the type description block in @internal/mcp/tools/create_document.go.
7. [x] Extend the valid-values description in @internal/mcp/tools/list_documents.go.
8. [x] Add the type list rows, the WHEN TO CREATE rows, the three selection rules, the relation conventions, and the status meanings in @internal/mcp/server.go.
9. [x] Cover registration, categories, counts, and template sections in @templates/templates_test.go.
10. [x] Cover `.feature` in @templates/source_extensions_test.go.

### Phase 2 — precision canon

11. [x] Add required sections for both types in @templates/precision.go.
12. [x] Add both types to the prose-profile table in @templates/precision.go.
13. [x] Register Flows and Journeys as step sections and, with Examples, as observation sections in @templates/precision.go.
14. [x] Add the foreign-heading rows in both directions in @templates/precision.go.
15. [x] Add the per-type cap table `MaxBodyLines` fed by `MaxSpecBodyLines` in @templates/precision.go.
16. [x] Collect Given/When/Then lines in Flows, Journeys, and Examples as steps in @internal/advisory/precision.go.
17. [x] Add the Anchors-line check per Flows subsection in @internal/advisory/precision.go.
18. [x] Add the actor-subject check over Flows and Journeys steps in @internal/advisory/precision.go.
19. [x] Route the body-cap check through the per-type table in @internal/advisory/precision.go.
20. [x] Cover every new finding and every non-finding in @internal/advisory/precision_actor_subject_test.go.

### Phase 3 — advisory engines and ranking

21. [x] Add `scenario` to the injection ranking below `spec` and above `guide` in @internal/advisory/code_alignment.go.
22. [x] Pin `scenario` inclusion, its rank, and `journey` exclusion in @internal/advisory/code_alignment_test.go.
23. [x] Pin default search ranking and the `.feature` mention in @internal/mcp/tools/search_documents_test.go.
24. [x] Pin cascade behavior for `scenario depends_on spec` in @cmd/hook_post_tool_use_test.go.
25. [x] Pin restatement over `scenario implements journey` in @internal/advisory/restatement_test.go.

### Phase 4 — integration and documentation

26. [x] Add lifecycle scenarios for both types in @internal/mcp/integration/actor_subject_spec_test.go.
...
31. [x] Update the type counts and tables in @README.md.
...
The accepted document-model decision keeps templates in @templates/ and document operations in @internal/docs/. The procedure for adding a type is recorded in the linked building doc. Phase 1 precedes phase 2, which precedes phase 3; phase 4 verifies all three; phase 5 waits for the RFC status in the global repository.

Older binaries skip the new files and report them in `status` (@internal/docs/scan.go, @cmd/status.go). No data migration is needed. The sync server's acceptance of the two new type strings is not verified here. [assumption] The server accepts any type string the client sends.
