...
**Canonical source**: this guide mirrors the Archcore plugin's spec contract (`skills/_shared/spec-contract.md` in the plugin repo) and its ADR "Spec — Single Narrative, Generalized Sections, EARS + BCP 14 Notation". When that contract changes, update this guide and the template in @cli/templates/templates.go together.
...
One number now covers every path. The separate 120-line flagship allowance that `/archcore:init` carried for hotspot synthesis is folded into the default, so a synthesized spec and an authored one are measured alike. The engine reads @cli/templates/precision.go `MaxSpecBodyLines`; the finding is advisory and never blocks a write.
