...
- The declaration schema in `Settings.Globals` (@cli/internal/config/config.go).
- Path resolution and source-health classification in @cli/internal/config/globals.go (`ResolveGlobalPath`, `ResolveGlobalPathFrom`, `CheckGlobalDir`, `DescribeGlobalDirError`).
- The two-phase scan in @cli/internal/docs/scan.go (`Scan`, `ScanFull`).
- Source-health reporting in @cli/internal/docs/inspect.go (`InspectGlobals`, `GlobalState`).
- Source annotation and read-only enforcement across the MCP read/write tools.
- The pre-write hook guard in @cli/cmd/hook_write_guard.go, which calls the same write-path validator.
- MCP startup validation of declared globals in @cli/cmd/mcp.go.

The document model, the scan, and the path guards live in `internal/docs`; @cli/internal/mcp/tools/docs_bridge.go re-exports them under the short names the MCP tool handlers use (`scanDocuments`, `scanDocumentsFull`, `readDocumentContent`, `guardWritablePath`, `validateReadPath`, `annotateSource`, `LocalDocument`). The bridge exports no local-only scan; a caller that needs one calls `docs.ScanLocal` or `docs.ScanLocalTypes` directly. See @.archcore/cli/docs-package-owns-the-document-model.adr.md.
...
- Write-path guard: @cli/internal/docs/guard.go (`GuardWritablePath`, and the package-private `checkSymlinkContainment` it layers) — consumed by @cli/internal/mcp/tools/create_document.go, @cli/internal/mcp/tools/update_document.go, @cli/internal/mcp/tools/remove_document.go, and @cli/cmd/hook_write_guard.go
- Relation guard: @cli/internal/mcp/tools/add_relation.go (`guardWritablePath` on both endpoints)
- Local-only scan: @cli/internal/docs/scan.go (`ScanLocal`) — phase 1 only; never fails on a broken global
- Read-path validation: @cli/internal/docs/guard.go (`ValidateReadPath`) — `get_document` and the post-write precision advisory; admits declared external globals, hardened (§4.4)
- Source annotation on read: @cli/internal/mcp/tools/get_document.go
- Local-only CLI surfaces: @cli/cmd/status.go (`checkGlobalSources`), @cli/cmd/hooks_common.go (global content excluded from status and the session recap; the SessionStart `GLOBALS` block reports per-source metadata from `InspectGlobals`; degrade to local-only on a broken global, warn on empty/invalid settings, §6.4)
- Startup validation: @cli/cmd/mcp.go (`checkGlobals`)
...
4. The write tools MUST use the single shared validator `docs.GuardWritablePath(baseDir, relPath, globals)` (@cli/internal/docs/guard.go), which layers, in order:
...
An implementation conforms if it satisfies all MUST/MUST NOT statements in §§1–7, the invariants, and the error-handling rows, and passes the tests in @cli/internal/docs/guard_test.go, @cli/internal/docs/inspect_test.go, @cli/internal/mcp/tools/globals_test.go, @cli/internal/mcp/tools/globals_edge_test.go, @cli/internal/mcp/tools/globals_reserved_test.go, @cli/internal/mcp/tools/guard_writable_path_test.go, @cli/internal/mcp/integration/globals_test.go, @cli/internal/mcp/integration/globals_worktree_test.go, the globals cases in @cli/internal/config/config_test.go, @cli/internal/config/globals_test.go and @cli/internal/config/globals_anchor_test.go, the anchor queries in @cli/internal/git/main_checkout_test.go, and the surface tests in @cli/cmd/mcp_test.go, @cli/cmd/mcp_worktree_test.go, @cli/cmd/status_test.go, @cli/cmd/hooks_common_test.go, @cli/cmd/hooks_globals_block_test.go, @cli/cmd/hook_write_guard_test.go, and @cli/internal/advisory/precision_path_test.go.
