...
A path is global when `settings.json` declares it as a global source, or when it sits under the reserved `.archcore/global/` directory. `docs.IsGlobalPath` and `docs.IsReservedGlobalDir` in `@cli/internal/docs/globals.go` answer those two halves for every guard.
...
- `docs.IsGlobalPath`, `docs.IsReservedGlobalDir`, and their case-folding siblings (`@cli/internal/docs/globals.go`) back `docs.GuardWritablePath`, which serves the MCP write guards (`@cli/internal/mcp/tools/create_document.go`, `@cli/internal/mcp/tools/update_document.go`, `@cli/internal/mcp/tools/remove_document.go`), the relation guard (`@cli/internal/mcp/tools/add_relation.go`), and the pre-write hook guard (`@cli/cmd/hook_write_guard.go`).
- `docs.IsExternalGlobalDocument` (`@cli/internal/docs/globals.go`) covers requirement 9 — the paths `GuardWritablePath` cannot classify.
- `archcore status` filters to local documents (`@cli/cmd/status.go`); the SessionStart recap scans local only, and its `GLOBALS` block is built from `docs.InspectGlobals` counts, never from document content (`@cli/cmd/hooks_common.go`, `@cli/internal/docs/inspect.go`).
- The code-alignment injection includes globals and marks them (`@cli/internal/advisory/code_alignment.go`).
- Tests: `@cli/internal/docs/guard_test.go`, `@cli/internal/mcp/tools/globals_test.go`, `@cli/internal/mcp/integration/globals_test.go`, `@cli/cmd/status_test.go`, `@cli/cmd/hooks_common_test.go`, `@cli/cmd/hooks_globals_block_test.go`, `@cli/cmd/hook_write_guard_test.go`.