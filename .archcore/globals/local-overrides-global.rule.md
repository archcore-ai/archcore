...
- Read-only guards in `create_document`, `update_document`, and `remove_document` (`@cli/internal/mcp/tools/`) prevent writes to globals.
- `source_kind` and `read_only` are always present on scanned documents (`docs.Document` in `@cli/internal/docs/document.go`, aliased as `LocalDocument` in `@cli/internal/mcp/tools/docs_bridge.go`), and `search_documents` results carry the same four source fields (`source_id`, `source_kind`, `global`, `read_only`), so the distinction is machine-checkable across all three read tools.
- Reviewer and agent behavior: WHEN answering from a global document, the responder states whether a local override exists.
- The fixture `@cli/examples/07-local-overrides-global/` exercises this precedence: the local `error-handling.rule.md` overrides the same-slug rule from `@cli/examples/_global_/company-standards/`.
