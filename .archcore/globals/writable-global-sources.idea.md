...
- **Split the predicate.** Today the MCP write guards, the relation guard, and the
  pre-write hook guard all reach `docs.GuardWritablePath` (@cli/internal/docs/guard.go), which
  consults `docs.IsReadOnlyGlobalPath` (@cli/internal/docs/globals.go). Keep that predicate on
...
- **New addressing scheme for external writes.** `create_document`'s `directory` param is
  relative-within-primary and rejects `..` (@cli/internal/mcp/tools/create_document.go) — it
...
- **F2**: `remove_document` cleans the *primary's* manifest, not the global's
  (@cli/internal/mcp/tools/remove_document.go) → cross-repo dangling relations in the global's
