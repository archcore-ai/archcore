...
Source: `@cli/cmd/init.go` (`installAgents` loop, then `maybeInstallInstructions`).
...
Source: `@cli/internal/agents/agents.go` (`Detect` function), individual agent `DetectFn` in
`internal/agents/*.go`.
...
`@cli/internal/wiring/hooks_agents.go`.
