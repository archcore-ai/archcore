...
`resolveProjectRoot` (@cli/cmd/mcp_root.go) picks the root in strict precedence — `--project` flag, then `ARCHCORE_PROJECT_ROOT`, then `os.Getwd()` — and rejects a resolved root that sits inside a known plugin install cache.
...
Where a host *does* support interpolation, prevention beats detection: the Cursor MCP entry ships `--project ${workspaceFolder}` (@cli/internal/agents/mcp_helpers.go), so the root never depends on spawn cwd in the first place.
