...
- Deprecated: earlier CLI versions wrote an owned `.claude/rules/archcore.md`. Re-wiring now removes
  that file.
- Source: `@cli/internal/agents/instructions.go`, `@cli/cmd/instructions.go`.
...
- Source: `@cli/internal/agents/claude_code.go`, `@cli/internal/wiring/hooks_agents.go`,
  `@cli/internal/plugin/claude_settings.go`
...
- Plugin: no CLI mechanism. Cursor manages plugins in its UI, so the CLI prints a one-line instruction
  and runs no command
- Source: `@cli/internal/agents/cursor.go`, `@cli/internal/wiring/hooks_agents.go`
...
- Source: `@cli/internal/agents/gemini_cli.go`, `@cli/internal/wiring/hooks_agents.go`
...
- Source: `@cli/internal/agents/codex_cli.go`, `@cli/internal/wiring/hooks_agents.go`
...
- Source: `@cli/internal/agents/copilot.go`, `@cli/internal/wiring/hooks_agents.go`
...
- Source: `@cli/internal/agents/opencode.go`, `@cli/cmd/hook_dialect.go`, `@cli/cmd/hook_payload.go`
...
- Instruction file: `AGENTS.md` (fenced upsert)
- Source: `@cli/internal/agents/roo_code.go`
...
- Instruction file: `AGENTS.md` (fenced upsert)
- Source: `@cli/internal/agents/cline.go`

## Adding a new agent

1. Add a new `AgentID` constant in `@cli/internal/agents/agents.go`.
...
3. Set `InstructionsPath`, `WriteInstructions`, and `RemoveInstructions` with the helpers in
   `@cli/internal/agents/instructions.go`: `agentsMDInstructions*` for `AGENTS.md`, `geminiInstructions*`
...
4. Add the agent constructor to the `all` slice in `@cli/internal/agents/agents.go`.
5. Add `internal/agents/<name>_test.go`.
6. IF the agent supports hooks, THEN add a `hostDialect` row in `@cli/cmd/hook_dialect.go` describing its
...
7. IF the agent supports hooks, THEN add an `InstallXxxHooks` function in
   `@cli/internal/wiring/hooks_agents.go` covering all three events, and add the agent to
...
8. IF the agent's host flattens MCP tool names into a spelling not already folded, THEN add its prefix
   in `@cli/cmd/hook_payload.go`. An unfolded spelling does not merely skip the post-write checks — it
...
9. IF the agent's host names a file-mutation tool that carries no path argument, THEN add the key its
   patch or path arrives under in `@cli/cmd/hook_payload.go`. A missing key fails silently: the guard
...
Add its name to `archcoreMCPTools` in `@cli/cmd/hook_payload.go` in the same change.
