...
1. Add a failing test that reads the first 2,048 bytes of a two-source response — `@cli/internal/mcp/tools/search_recall_test.go`.
2. Reorder `searchDocumentsResult` to `coverage`, `hits`, `truncated`, `index`, `results` — `@cli/internal/mcp/tools/search_documents.go`.
...
6. Collect path-reference candidates; order them by specificity, then explicit before mention, then byte offset — `@cli/internal/mcp/tools/search_documents.go`.
...
9. Update `TestHandleSearchDocuments_PathRefRepetitionIsNotRelevance`, which asserts ten matches — `@cli/internal/mcp/tools/search_documents_test.go`.
10. Amend §5.6, §5.7, §10, and the Constraints table — `@.archcore/mcp/search-documents.spec.md`.

### Phase 3 — Byte budget

11. Add `jsonStringSize` with a table test and a fuzz test against `json.Marshal` — new `@cli/internal/mcp/tools/response_budget.go`.
12. Declare `searchResponseByteBudget` and `searchBodyFloorBytes`; each doc comment names the budget it protects — `@cli/internal/mcp/tools/search_documents.go`.
...
16. Add the incident regression test: six bodies of two-byte characters, two sources, `mode=full`, `limit=6` — new `@cli/internal/mcp/tools/response_budget_test.go`.
...
19. Replace `searchDocumentsDescription` and the `content`, `mode`, and `limit` parameter texts; keep the text host-neutral — `@cli/internal/mcp/tools/search_documents.go`.
20. Replace the GLOBAL SOURCES paragraph in `buildInstructions` — `@cli/internal/mcp/server.go`.
21. Amend clause 13 to the new precedence sentence — `@.archcore/globals/session-globals-disclosure.spec.md`.
22. Change `globalsPrecedenceLine` and its test to the amended sentence — `@cli/cmd/hooks_common.go`, `@cli/cmd/hooks_globals_block_test.go`.
...
26. Add the slug to `scoreContent` after the title, at specificity 3, in every match mode — `@cli/internal/mcp/tools/search_documents.go`.
27. Fold the six separators in query words and searched text under `all` and `any`; reject a word that folds to nothing.
28. Add the slug, convergence, exact-mode, and separator-only tests; update `TestHandleSearchDocuments_ContentBodyHit` — `@cli/internal/mcp/tools/search_recall_test.go`, `@cli/internal/mcp/tools/search_documents_test.go`.
