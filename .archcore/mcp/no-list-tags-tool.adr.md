...
1. **Session context injection** — `buildSessionContext()` in `cmd/hooks_common.go` injects `EXISTING TAGS: backend, auth, frontend, ...` at MCP session start. Top tags by frequency, no counts, comma-separated; the cap is `maxSessionTags` in `@cli/cmd/hooks_common.go`. Provides proactive discoverability without the agent needing to know to ask.
