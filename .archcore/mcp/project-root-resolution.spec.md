...
It is normative for the root provider (@cli/internal/mcp/root_provider.go), for the acceptance checks a candidate root must pass, and for the messages a refusal emits. It supersedes nothing in @.archcore/integrations/host-cwd-misrouting.adr.md; the plugin-cache guard stays a required check. The originating decision is @.archcore/mcp/session-following-project-root.adr.md.
