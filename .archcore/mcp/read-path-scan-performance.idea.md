...
@cli/internal/docs/scan.go). Since 2026-07 it has an **mtime+size-keyed
per-file cache** (@cli/internal/docs/cache.go): the walk still runs every
...
manifest through a cached store (@cli/internal/mcp/tools/manifest_store.go) keyed on
...
- @cli/internal/mcp/tools/scaling_bench_test.go — synthetic 1.8 KB bodies, no relations.
- @cli/internal/mcp/tools/realistic_bench_test.go — corpora replicating this repo's
...
1. **Scan cache keyed by mtime.** ✅ Shipped 2026-07, now at
   @cli/internal/docs/cache.go — (mtime, size)-keyed per-file cache,
