...
**Shipped 2026-04-07** in commit `df1015e`. Both checks live in `@cli/cmd/status.go`. They run in
...
only finding, which `@cli/cmd/status.go` records in an inline comment.

The predicted noise is observable. In `@cli/examples/`, the projects `03-product-planning`,
...
Tag format is already validated inline by `@cli/cmd/status.go`, which rejects a malformed tag. Neither
