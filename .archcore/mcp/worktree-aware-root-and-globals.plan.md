...
1. [x] Add the working-tree queries to @cli/internal/git/git.go: `Toplevel`, `MainCheckout`, and the `WorktreeRoots` pair, all over the existing `run` with its 500 ms bound.
2. [x] Add `ResolveGlobalPathFrom` to @cli/internal/config/globals.go as the pure core, and classify a cleaned relative path as in-tree or escaping in `ResolveGlobalPath`.
...
7. [x] Add table-driven cases to @cli/internal/config/globals_anchor_test.go and @cli/internal/git/main_checkout_test.go for a main checkout, a linked worktree, a nested project, a submodule, a non-git directory, and a machine without git.
8. [x] Add the worktree fixture in @cli/internal/mcp/integration/globals_worktree_test.go beside `setupPrimaryWithSiblingGlobal`.
9. [x] Add the startup cases in @cli/cmd/mcp_worktree_test.go: a worktree that resolves only from the main checkout, and a source absent from both anchors that stays fatal.

Two things the implementation corrected against the original task list. Anchoring on the main checkout root broke every nested fixture under @cli/examples/, so the anchor maps the primary's relative position instead. Threading the anchor through the scan and the reporter proved unnecessary: every consumer already funnels through `config.ResolveGlobalPath`, so a memoized lookup behind one seam replaced tasks 4 and 5 as originally written.

### Phase 2 — session-following root (#31) — done

10. [x] Add `sessionRootProvider` in @cli/internal/mcp/root_provider.go, querying `roots/list` through `server.ClientSessionFromContext` and `SessionWithRoots`, under a 500 ms timeout and a 2 s decision cache.
...
12. [x] Implement the acceptance checks of @.archcore/mcp/project-root-resolution.spec.md, and move `isPluginCachePath` to @cli/internal/projectroot/ so the start-time resolver and the provider share one guard.
13. [x] Add `file://` parsing with percent-decoding, host rejection, and Windows drive handling.
14. [x] Change the ten tool constructors in @cli/internal/mcp/server.go to take `tools.RootProvider`, and resolve the root at the top of each handler body.
15. [x] Add `tools.StaticRoot` so every per-tool unit test is a one-token change rather than a rewrite.
16. [x] Add `baseDir` to `HostWiringFunc` in @cli/internal/mcp/tools/install_host_config.go and drop the closure in @cli/cmd/host_wiring.go.
17. [x] Add `WithPinnedRoot` and `WithRootWarnings`, and pin the root in @cli/cmd/mcp.go when `--project` or `ARCHCORE_PROJECT_ROOT` is set.
18. [x] Emit one warning line per distinct refusal reason, with no absolute path.
19. [x] Add a roots-capable in-process client in @cli/internal/mcp/integration/roots_test.go, built with both `transport.WithRootsHandler` and `client.WithRootsHandler`, and drive every conformance case the spec lists.
20. [x] Correct the "one MCP server process serves one primary" comment in @cli/internal/mcp/tools/manifest_store.go.
21. [x] Confirm @cli/cmd/project_root_flag_test.go still holds: `resolveProjectRoot` keeps its shape, and the provider sits above it.
22. [x] Confirm no invalidation is needed for the main-checkout memo of @cli/internal/config/globals.go: it is keyed by project root, and a root's anchor cannot change while the process runs.
...
- @.archcore/integrations/host-cwd-misrouting.adr.md documents the three properties every plugin-cache fragment must keep; they moved intact into @cli/internal/projectroot/.
