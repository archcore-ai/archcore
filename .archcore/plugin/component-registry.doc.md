...
`@test/structure/codex-plugin.bats` enforces that every entry exists, carries `description:`, and references the matching `skills/<name>/SKILL.md`. `@test/structure/copilot-plugin.bats` enforces that the wrapper set matches the skill set and that the manifest points at `commands/` — Copilot gives that field no default path, so without the pointer the entire `/archcore:*` surface is missing there. Claude Code and Cursor need no wrappers, because they surface skills directly. `@test/structure/command-grammar.bats` pins each argument hint to its mode list.
