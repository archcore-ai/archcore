...
Publish one release of the plugin and the CLI from one git tag on `archcore-ai/archcore`. The tag is the only version source: the release writes it into the four published plugin manifests and into the CLI binary.
...
Do not edit `version` in the four manifests under `plugin/plugins/archcore/`. They stay at `0.0.0` on `dev`, and @plugin/test/structure/manifest-version-parity.bats fails on any other value.
...
3. Confirm that `Plugin Tests` and `CLI Tests` are green on the `dev` head.
...
5. Monitor the `Release` run at `https://github.com/archcore-ai/archcore/actions`.
...
6. Verify the published release.
   ...
   Expected result: 9 assets on the release page (6 archives, `checksums.txt`, `install.sh`, `install.ps1`), the plugin layout on `main`, `X.Y.Z` from the published manifest, and `vX.Y.Z` from `archcore --version`.
...
- `main` carries only the exported plugin layout, and its four manifests carry the tag's version.
...
- `verify-version` fails: the tag is not `vMAJOR.MINOR.PATCH`, or it skips, repeats, or goes back a version. The error names the three accepted versions. Delete the tag and push an accepted version.
...
- A test job fails: nothing was published. Fix on `dev`, delete the tag as above, and tag the fixed commit.
