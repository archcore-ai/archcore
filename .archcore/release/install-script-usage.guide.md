...
The CLI resolves the same path through `StateDir` in `@cli/internal/xdg/xdg.go`, which `@cli/internal/telemetry/telemetry.go` and the freshness cache in `@cli/internal/update/cache.go` both call. The `.local/state` layout applies on every platform including Windows. Do not move the identifier to a platform-idiomatic location such as `%LOCALAPPDATA%`. The CLI's own telemetry reads the same identifier and joins "installed" to "used" without a second identifier.
...
`@cli/install.sh` and `@cli/install.ps1` in this repository carry a `__POSTHOG_KEY__` placeholder in place of a PostHog project key. The `archcore-ai/landing` deploy workflow substitutes the real key while it copies the scripts into `public/`. Both installers send an event only when the key has the `phc_` prefix.
...
The CLI applies the same rule to its own key, which `@cli/.goreleaser.yaml` injects at link time. A build without an injected key sends nothing, so `go build` and a fork's pipeline both produce a silent binary. `@cli/scripts/assert-not-inert.sh` fails the release when the official pipeline produces one by accident.
