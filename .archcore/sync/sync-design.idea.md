...
metadata. `@cli/internal/sync/manifest.go` defines the structure and its validation.
...
`Client` in `@cli/internal/api/client.go` sets the `Authorization: Bearer <token>` header from its
...
- `@cli/internal/sync/manifest.go` — `Manifest` struct, load and save, validation
- `@cli/internal/sync/hash.go` — SHA-256 hashing, file scanning
- `@cli/internal/sync/diff.go` — change detection: created, modified, deleted, unchanged
- `@cli/internal/sync/payload.go` — payload construction, frontmatter parsing
- `@cli/internal/api/client.go` — sync client constructor, `applyAuth`, `Sync()`
- `@cli/cmd/sync.go` — cobra command, preconditions, flow orchestration
