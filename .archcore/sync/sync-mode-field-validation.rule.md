...
- `Settings.Validate` and the custom `MarshalJSON` in `@cli/internal/config/config.go` implement the matrix.
- Tests: the validation cases in `@cli/internal/config/config_test.go`.
