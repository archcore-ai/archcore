...
1. The CLI MUST validate every file path in a sync payload and in the manifest against path traversal. `validateRelPath()` in `@cli/internal/sync/payload.go` performs this check at every boundary.
...
5. A manifest entry MUST NOT contain `//` and MUST NOT end with `/`. `validateFileEntry` in `@cli/internal/sync/manifest.go` performs this check.
6. `ScanFiles` in `@cli/internal/sync/hash.go` MUST walk `.archcore/` through the shared `WalkArchcoreFilesSkipping`, which skips hidden directories and meta files.
...
- `validateRelPath` in `@cli/internal/sync/payload.go` and `validateFileEntry` in `@cli/internal/sync/manifest.go` reject an invalid path before any file read or manifest write.
- Tests: `@cli/internal/sync/payload_test.go`, `@cli/internal/sync/manifest_test.go`, and `@cli/internal/sync/hash_test.go` cover the traversal, absolute-path, and skip-list cases.
