...
installed. Today `objectNamesPlugin` and `findPluginEntry` in `@cli/internal/plugin/evidence.go` count an
entry the host marks `"installed": false`, and count a bare marketplace id carried in an identity
value. The key reading in the same file and `registryNamesPlugin` in `@cli/internal/plugin/registry.go`
...
1. Read the `installed` boolean of a listing entry in `objectNamesPlugin`; a `false` answers "not installed" — `@cli/internal/plugin/evidence.go`.
2. Narrow the identity-value match to the plugin itself; exclude the bare marketplace id, as the key reading does — `@cli/internal/plugin/evidence.go`.
3. Apply the same boundary to the plain-text fields in `parseTextListing` — `@cli/internal/plugin/evidence.go`.
4. Stop the walk from descending into an entry the host already answered for — `@cli/internal/plugin/evidence.go`.
5. Name the guard classification at the branch that decides absence, per `@.archcore/code-quality/fail-open-or-fail-closed-reads.rule.md` — `@cli/internal/plugin/evidence.go`.

### Phase 2 — Tests

6. Add a JSON case: an entry under `available` carrying `"installed": false` answers not listed — `@cli/internal/plugin/evidence_test.go`.
7. Add a JSON case: an identity value equal to the marketplace id answers not listed — `@cli/internal/plugin/evidence_test.go`.
8. Add a text case: a marketplace-named line without an installed plugin answers not listed — `@cli/internal/plugin/evidence_test.go`.
9. Add a planner case: that evidence plans no update action and no already-installed report — `@cli/internal/plugin/plan_test.go`.
...
- The branch that decides absence in `@cli/internal/plugin/evidence.go` names its guard classification and cites the rule slug.
...
- The host listing schemas are another project's to change; the walk stays tolerant of the envelope — `@cli/internal/plugin/evidence.go` records that as an assumption.
